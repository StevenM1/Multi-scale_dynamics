...
makeSamplers <- function(decisionModel, learningModel, dctModel, task, samples_dir='./samples',
                         trendModel='NULL', n_cosines=0, ds4_with_baseline=FALSE,
                         n_chains=3, dat=NULL, do_return=FALSE, dct_highpass=1/400, overwrite=FALSE) {

  if(ds4_with_baseline) {
    save_fn_samples <- file.path(samples_dir, paste0(task, '_model-', decisionModel, '-', learningModel, '_with_baseline-DCT-', dctModel, '.RData'))
  } else {
  ...
  # if(trendModel != 'NULL') {
  #   save_fn_samples <- file.path(samples_dir, paste0(task, '_model-', decisionModel, '-', learningModel, '-exptrend-', trendModel, '.RData'))
  # } else {
  #   save_fn_samples <- file.path(samples_dir, paste0(task, '_model-', decisionModel, '-', learningModel, '-DCT-', dctModel, '.RData'))
  # }
    ...
    if(n_cosines == 0) {
      ...
      if(dct_highpass==1/250) {
        if(task == 'wagenmakers2008exp2') {
          ...
          n_cosines = ncol(makeDCT(1:averageNTrials, high_pass=1/250))
        }
      } else {
        n_cosines = ncol(makeDCT(1:averageNTrials, high_pass=dct_highpass))
        if(task == 'wagenmakers2008exp2') n_cosines = 7
      }
    }

    if(dctModel == 'B') {
      ## drift only affects thresholds, so set effect on drift rates to 0
      constants <- c(constants, setNames(rep(0, n_cosines), paste0('vcos', 1:n_cosines)))
      constants <- c(constants, setNames(rep(0, n_cosines), paste0('ucos', 1:n_cosines)))
      Flist2 <- lapply(paste0(paste0('Bcos', 1:n_cosines),'~1'), as.formula) # cosines for DCT
      Flist <- append(Flist, Flist2)
    } else if(dctModel == 'v') {
      ## drift only affects rates, so set effect on thresholds to 0
      constants <- c(constants, setNames(rep(0, n_cosines), paste0('Bcos', 1:n_cosines)))
      constants <- c(constants, setNames(rep(0, n_cosines), paste0('ucos', 1:n_cosines)))
      Flist2 <- lapply(paste0(paste0('vcos', 1:n_cosines),'~1'), as.formula) # cosines for DCT
      Flist <- append(Flist, Flist2)
    } else if(dctModel == 'u') {
      constants <- c(constants, setNames(rep(0, n_cosines), paste0('Bcos', 1:n_cosines)))
      constants <- c(constants, setNames(rep(0, n_cosines), paste0('vcos', 1:n_cosines)))
      Flist2 <- lapply(paste0(paste0('ucos', 1:n_cosines),'~1'), as.formula) # cosines for DCT
      Flist <- append(Flist, Flist2)
    }
  }
  ...
  if(length(nuisance_non_hyper) > 0) {
    ...
    prior <- list(
      ...
      prior_nuis=list(
        ...
        theta_mu_var = diag(rep(0.01, length(nuisance_non_hyper)))
      )
    )
    } else {
      prior <- list(
