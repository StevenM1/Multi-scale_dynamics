...
addPreviousTrialInfo <- function(dat, dataColumn='S', nHistory=3, repeatAlternate=FALSE) {
  ...
  for(i in (nHistory+1):0) {
    dat[,paste0('Sminus', i)] <- Lag(dat[,dataColumn], i)
  }
...
wagenmakers2004_CS <- loadDataAndPP('wagenmakers2004_CS', 'zSMuAHbV', 'NULL')
forstmann2008 <- loadDataAndPP('forstmann2008', 'zSMuAHbV', 'NULL')
mileticvanmaanen2019exp2block2 <- loadDataAndPP('mileticvanmaanen2019exp2block2', 'zSMuAHbV', 'NULL')
wagenmakers2008exp2 <- loadDataAndPP('wagenmakers2008exp2', 'zSMuAHbV', 'NULL')
...
for(colors_ in c('black', 'white')) {
  if(colors_ == 'black') {
    ...
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_stimuluscoded.pdf'), width=8, height=5)
  }
...
# Group, by RA
pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_RAcoded.pdf'), width=8, height=5)
...
for(colors_ in c('black', 'white')) {
  if(colors_ == 'black') {
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_individual_RAcoded_ordered_black.pdf'), width=8, height=5)
    if(colors_ == 'black') par(bg = 'black', fg='white', col='white', col.axis='white', col.main='white', col.lab='white', col.sub='white')
  } else {
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_individual_RAcoded_ordered.pdf'), width=8, height=5)
  }
...
wagenmakers2004_CS <- loadDataAndPP('wagenmakers2004_CS', 'zSM', 'NULL')
forstmann2008 <- loadDataAndPP('forstmann2008', 'zSM', 'NULL')
mileticvanmaanen2019exp2block2 <- loadDataAndPP('mileticvanmaanen2019exp2block2', 'zSM', 'NULL')
wagenmakers2008exp2 <- loadDataAndPP('wagenmakers2008exp2', 'zSM', 'NULL')
...
for(colors_ in c('black', 'white')) {
  if(colors_ == 'black') {
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_stimuluscoded_smonly_black.pdf'), width=8, height=5)
    if(colors_ == 'black') par(bg = 'black', fg='white', col='white', col.axis='white', col.main='white', col.lab='white', col.sub='white')
  } else {
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_stimuluscoded_smonly.pdf'), width=8, height=5)
  }
  par(mfcol=c(2,4), bty='l', oma=c(0,2,0,0))
  plotSequentialEffectsGroup(dat=wagenmakers2004_CS$dat, pp=wagenmakers2004_CS$pp, seqEffectsSummary=seqEffectsSummary1,
                             nHistory = 3, main1='Dataset 1', ylab1='RT (s)', ylab2='Choice proportion', full.legend=FALSE)
...
for(fn in c('pdf', 'jpeg')) {
  if(fn == 'pdf') pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_and_individual_smonly.pdf'), width=8, height=8)
  if(fn == 'jpeg') jpeg(file=file.path(figures_dir, 'stimulus_memory_effects_group_and_individual_smonly.jpeg'), width=8, height=8, units='in', quality=100, res=500)
  l <- layout(m)
  #layout.show(l)
...
##
wagenmakers2004_CS <- loadDataAndPP('wagenmakers2004_CS', 'zSMuAH', 'NULL')
forstmann2008 <- loadDataAndPP('forstmann2008', 'zSMuAH', 'NULL')
mileticvanmaanen2019exp2block2 <- loadDataAndPP('mileticvanmaanen2019exp2block2', 'zSMuAH', 'NULL')
wagenmakers2008exp2 <- loadDataAndPP('wagenmakers2008exp2', 'zSMuAH', 'NULL')
...
for(colors_ in c('black', 'white')) {
  if(colors_ == 'black') {
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_stimuluscoded_smahonly_black.pdf'), width=8, height=5)
    if(colors_ == 'black') par(bg = 'black', fg='white', col='white', col.axis='white', col.main='white', col.lab='white', col.sub='white')
  } else {
    pdf(file=file.path(figures_dir, 'stimulus_memory_effects_group_stimuluscoded_smahonly.pdf'), width=8, height=5)
  }
  par(mfcol=c(2,4), bty='l', oma=c(0,2,0,0))
  plotSequentialEffectsGroup(dat=wagenmakers2004_CS$dat, pp=wagenmakers2004_CS$pp, seqEffectsSummary=seqEffectsSummary1,
                             nHistory = 3, main1='Dataset 1', ylab1='RT (s)', ylab2='Choice proportion', full.legend=FALSE)
...
for(ftype in c('jpeg','pdf')) {
  fn = file.path(figures_dir, 'stimulus_memory_effects_group_and_individual_dataset5_smonly.pdf')
