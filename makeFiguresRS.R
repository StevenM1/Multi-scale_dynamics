...
rm(list=ls())
decisionModel <- 'RDM'
learningModel <- c('NULL', 'bV', 'zSMuAHbV', 'zSMaAHbV')[3]
dctModel <- 'NULL'
task <- c('wagenmakers2004_CS', 'forstmann2008', 'mileticvanmaanen2019exp2block2', 'wagenmakers2008exp2', 'mileticvanmaanen2019exp2block1')[5]
...
get_pp2 <- function(task, learningModel, dctModel=NULL, get_pp=FALSE) {
...
#  plot_chains(samplers, filter=filter, selection='mu', layout=c(3,3))
  ...
  pp2 <- post_predict(samplers, n_cores = 10, filter=filter) ## subfilter=nchains[1,4]-1000)
  ...
  return(list(samplers=samplers, pp2=pp2, dat=dat, pp=pp))
}
...
get_aggregates_single_subject <- function(subject, samplers, pp, npost=100) {
  ...
  out = do.call(rbind, lapply(1:npost, function(x) {
    p_vector <- attr(pp, 'pars')[[x]][subject,]
    parsByTrial <- EMC2:::get_pars(p_vector, dadm)
    q <- parsByTrial[,c('q03')]
...
get_stats <- function(pp2, da, use_median=FALSE, flip=FALSE) {
  ...
  # acc_pp <- aggregate(accuracy.x~D*postn*subjects, combined_data, mean)
  # acc_pp <- aggregate(accuracy.x~D*subjects, acc_pp, quantile, c(0.025, 0.5, .975))
  # acc_pp <- aggregate(accuracy.x~D, acc_pp, mean)
  #
  # acc_data <- aggregate(accuracy~D*subjects, da, mean)
  # acc_data <- aggregate(accuracy~D, acc_data, mean)
...
dataset5 <- get_pp2('mileticvanmaanen2019exp2block1', 'zSMuAHbV', 'NULL')
dat5 <- prep_data(dataset5$dat)
pp5 <- prep_data(dataset5$pp2)
...
aggregates1 <- with(wagenmakers2004_CS, get_da_with_Q3bins(samplers, pp2))
aggregates2 <- with(forstmann2008, get_da_with_Q3bins(samplers, pp2))
aggregates3 <- with(mileticvanmaanen2019exp2block2, get_da_with_Q3bins(samplers, pp2))
aggregates4 <- with(wagenmakers2008exp2, get_da_with_Q3bins(samplers, pp2))
aggregates5 <- with(dataset5, get_da_with_Q3bins(samplers, pp2))
...
for(ftype in c('jpeg','pdf')) {
  fn = './figures/bV_effect_objective_subjective_combined.pdf'
