...
if(length(args)>0) {
  ...
  dctModel <- args[3]
  task <- args[4]
  cores_for_chains = args[5]
  cores_per_chain = args[6]
  trendModel <- args[7]  # THIS IS NEW
  if(length(args) == 8) max_trys <- as.integer(args[8]) else max_trys <- 15
...
# Set-up directories
print(paste('Running:', decisionModel, learningModel, dctModel, trendModel, task, n_cosines))
...
if(file.exists(save_fn_samples)) {
  ...
  source(file.path(root_dir, 'makeSamplers.R'))
  makeSamplers(decisionModel=decisionModel, learningModel=learningModel, dctModel=dctModel, trendModel=trendModel, task=task, samples_dir=samples_dir, n_cosines=n_cosines, n_chains=3)
}
...
#  if(learningModel != 'NULL') {
    ...
    if(!samplers[[1]]$init & dctModel=='NULL' & trendModel=='NULL') {  # NB: nuisance is an issue at the moment
...
#    }
  ...
  fn_ <- file.path(figure_dir, paste0('model-', decisionModel, '-', learningModel, '_dct-', dctModel, '_XXXX.pdf'))
  ...
  sink(gsub("XXXX.pdf", 'diagnostics.txt', fn_))
  print(all)
  sink()
}
