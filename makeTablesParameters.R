...
get_pp <- function(task, learningModel, dctModel='NULL',decisionModel='RDM') {
  # Load all ----------------------------------------------------------------
...
decisionModel <- 'RDM'
learningModel <- c('NULL', 'zSMuAHbV', 'zSMaAHbV')[2]
dctModel <- 'NULL'
...
samplers1 <- get_pp('wagenmakers2004_CS', learningModel, dctModel = dctModel)$samplers
samplers2 <- get_pp('forstmann2008', learningModel, dctModel = dctModel)$samplers
samplers3 <- get_pp('mileticvanmaanen2019exp2block2', learningModel, dctModel = dctModel)$samplers
samplers4 <- get_pp('wagenmakers2008exp2', learningModel, dctModel = dctModel)$samplers

get_theta_mu_values <- function(samplers) {
  samples <- merge_samples(samplers)
  idx <- samples$samples$stage=='sample'
  Ntransform <- attr(samplers[[1]]$data[[1]],'model')()$Ntransform

  theta_mu_transformed <- data.frame(Ntransform(t(samples$samples$theta_mu[,idx])))
  theta_mu_transformed <- theta_mu_transformed[,!grepl('cos', colnames(theta_mu_transformed))]
  theta_mu_transformed <- theta_mu_transformed[,c('alpha1', 'weight1', 'alpha2', 'weight2', 'alpha3', 'weight3')]
  colnames(theta_mu_transformed) <- c('alpha_SM', 'weight_SM', 'alpha_AH', 'weight_AH', 'alpha_RS', 'weight_RS')
...
get_alpha_values <- function(samplers, return_range=TRUE) {
  ...
  # automatic mapping doesn't work ehre due to my changes in EMC
  alphas <- EMC2:::parameters_data_frame(samplers, filter='sample', mapped=FALSE, selection='alpha')
  #alphas[,c('alpha1', 'alpha2','alpha3')] <-
  alphas <- data.frame(cbind(subjects=alphas[,1], Ntransform(as.matrix(alphas[,2:ncol(alphas)]))))
  alphas <- alphas[,c('subjects', 'alpha1', 'weight1', 'alpha2', 'weight2', 'alpha3', 'weight3')]
  ...
  # else apply(agg, 2, quantile, c(0.025, 0.5, .975))
}

out = lapply(list(samplers1, samplers2, samplers3, samplers4), get_theta_mu_values)
...
out2 = lapply(list(samplers1, samplers2, samplers3, samplers4), get_alpha_values)
...
write.csv(tabl, file='./tables/parameter_tables/parameter_values_incl_range.csv')
write.xlsx(tabl, file='./tables/parameter_tables/parameter_values_incl_range.xlsx', colNames = TRUE, rowNames = TRUE)
