...
plot_4_tasks_cdfs <- function(tasks, margin_text='') {
  for(taskn in 1:length(tasks)) {
    task <- tasks[taskn]
    dat <- EMC2:::loadRData(paste0('./datasets/', task, '.RData'))
    fns <- get_fns(task, learningModel='NULL', trendModel='NULL', samples_dir='./samples/')
    pp_fn <- fns[[1]]; samplers_fn <- fns[[2]]
    pp <- EMC2:::loadRData(pp_fn)
    samplers <- EMC2:::loadRData(samplers_fn)
...
ftype <- 'jpeg'
if(ftype == 'pdf') pdf(file='./figures/spectra_all_full_x_notrends_vs_trends.pdf', width=8, height=3)
if(ftype == 'jpeg') jpeg(file='./figures/spectra_all_full_x_notrends_vs_trends.jpeg', width=8, height=3, units='in', quality=100, res=100)
...
# What about MVM exp 2 block 1?
pp <- EMC2:::loadRData('./samples/mileticvanmaanen2019exp2block1_model-RDM-zSMuAHbV-DCT-NULL_pp.RData')
pp$rt <- pmin(pp$rt, 3) # Censor, like data
data <- EMC2:::loadRData('./datasets/mileticvanmaanen2019exp2block1.RData')

ftype <- 'jpeg'
if(ftype == 'pdf') pdf(file='./figures/spectra_ds5.pdf', width=6, height=3)
if(ftype == 'jpeg') jpeg(file='./figures/spectra_ds5.jpeg', width=6, height=3, units='in', quality=100, res=100)
...
par(mgp=c(3,1,0))
mar=c(4,3,2,0.5)
plotSpectrum(dat=data,pp=pp,dat_fn='./datasets/mileticvanmaanen2019exp2block1.RData',pp_fn='./samples/mileticvanmaanen2019exp2block1_model-RDM-zSMuAHbV-DCT-NULL_pp.RData',trial_duration = mean(data$rt)+0.8,xlab = 'Period',ylab = 'Log power', main='B. Spectrum')
