...
for(model in c(model1, model2)) {
  if(model == model1) {list2env(MS3,globalenv()); modelName='MS3RDM'}
  if(model == model2) {list2env(MS2,globalenv()); modelName='MS2RDM'}

  for(ftype in c('pdf','jpeg')) {
    if(ftype == 'pdf') pdf(file=file.path(figures_dir, paste0('ER_', modelName, '_group_and_individual.pdf')), width=8.5, height=5)
    if(ftype == 'jpeg') jpeg(file=file.path(figures_dir, paste0('ER_', modelName, '_group_and_individual.jpeg')), width=8.5, height=5, units='in', res=500)
    l <- layout(m)
    #layout.show(l)
    ...
    plotPES(distributions=distributions1, dat=wagenmakers2004_CS$dat, pp2=NULL, averageOnly=FALSE, plotAccuracy = FALSE, setPar=FALSE, plotAxisTickLabels = FALSE, orderSubjects = TRUE, meanOnly=TRUE, xlab1='', main1='Dataset 1')
    mtext('Participant', side=1, line=1, cex=.66)
    abline(h=0, col=adjustcolor(1, alpha.f=.5))
    mtext('B. RT difference (s)', side=2, cex=.66, line=2)
    plotPES(distributions=distributions2, dat=forstmann2008$dat, pp2=NULL, averageOnly=FALSE, plotAccuracy = FALSE, setPar=FALSE, plotAxisTickLabels = FALSE, orderSubjects = TRUE, meanOnly=TRUE, xlab1='Participant', main1='Dataset 2')
    mtext('Participant', side=1, line=1, cex=.66)
    abline(h=0, col=adjustcolor(1, alpha.f=.5))
    plotPES(distributions=distributions3, dat=mileticvanmaanen2019exp2block2$dat, pp2=NULL, averageOnly=FALSE, plotAccuracy = FALSE, setPar=FALSE, plotAxisTickLabels = FALSE, orderSubjects = TRUE, meanOnly=TRUE, xlab1='Participant', main1='Dataset 3')
    mtext('Participant', side=1, line=1, cex=.66)
    abline(h=0, col=adjustcolor(1, alpha.f=.5))
    plotPES(distributions=distributions4, dat=wagenmakers2008exp2$dat, , pp2=NULL, averageOnly=FALSE, plotAccuracy = FALSE, setPar=FALSE, plotAxisTickLabels = FALSE, orderSubjects = TRUE, meanOnly=TRUE, xlab1='Participant', main1='Dataset 4')
    mtext('Participant', side=1, line=1, cex=.66)
    abline(h=0, col=adjustcolor(1, alpha.f=.5))
    dev.off()
  }
  ...
  for(colors_ in c('black', 'white')) {
    if(colors_ == 'black') {
      pdf(file=file.path(figures_dir, paste0('ER_', modelName, '_group_black.pdf')), width=8.5, height=2.75)
      if(colors_ == 'black') par(bg = 'black', fg='white', col='white', col.axis='white', col.main='white', col.lab='white')
    } else {
      pdf(file=file.path(figures_dir, paste0('ER_', modelName, '_group.pdf')), width=8.5, height=2.75)
    }
  ...
  for(colors_ in c('black', 'white')) {
    if(colors_ == 'black') {
      pdf(file=file.path(figures_dir, paste0('ER_', modelName, '_individual_black.pdf')), width=8, height=2.75)
      if(colors_ == 'black') par(bg = 'black', fg='white', col='white', col.axis='white', col.main='white', col.lab='white')
    } else {
      pdf(file=file.path(figures_dir, paste0('ER_', modelName, '_individual.pdf')), width=8, height=2.5)
    }
    layout(matrix(1:4, ncol=4), widths=c(.15, .25, .5, .2))
    par(mar=c(2,2,3,.5), mgp=c(2,.75,0), bty='l', oma=c(0,1,0,0))
...
#   if(ftype == 'pdf') pdf(file=file.path(figures_dir, paste0('ER_MS2RDM_and_MS3RDM_group_and_individual.pdf')), width=8.5, height=7)
#   if(ftype == 'jpeg') jpeg(file=file.path(figures_dir, paste0('ER_MS2RDM_and_MS3RDM_group_and_individual.jpeg')), width=8.5, height=7, units='in', res=500, quality=100)
...
for(ftype in c('jpeg', 'pdf')) {
  if(ftype == 'pdf') pdf(file=file.path(figures_dir, paste0('ER_MS2RDM_and_MS3RDM_group_and_individual.pdf')), width=8.5, height=8.5)
  if(ftype == 'jpeg') jpeg(file=file.path(figures_dir, paste0('ER_MS2RDM_and_MS3RDM_group_and_individual.jpeg')), width=8.5, height=8.5, units='in', res=500, quality=100)
...
## Group zSMuAH overlapping with zSMuAHbV -- supplementary figure
pdf(file='./figures/ER_MS2RDM_and_MS3RDM_preerrorspeeding.pdf', width=8, height=2.75)
par(mgp=c(2,1,0), bty='l', mar=c(3,2,2,.5), oma=c(0,2,0,0),mfrow=c(1,3))
plotPES2(distributions=MS3$distributions1, dat=wagenmakers2004_CS$dat, pp2=NULL, shift=TRUE, main1='Dataset 1', rtmeasure = 'rtabs', accmeasure='accabs', ylab1='RT (s)', ylab='Accuracy', plotAccuracy = FALSE, meanOnly=TRUE, setPar = FALSE)
plotPES2(distributions=MS2$distributions1, dat=wagenmakers2004_CS$dat, do.plot=FALSE, pp2=NULL, main1='Dataset 1', rtmeasure = 'rtabs', accmeasure='accabs', ylab1='RT (s)', ylab='Accuracy', plotAccuracy = FALSE, meanOnly=TRUE, setPar = FALSE)
mtext('RT (s)', side=2, cex=.66, line=2)
mtext('', side=2, cex=par()$cex*par()$cex.main, line=3, font=2)

plotPES2(distributions=MS3$distributions3, dat=mileticvanmaanen2019exp2block2$dat, pp2=NULL, shift=TRUE, main1='Dataset 3', rtmeasure = 'rtabs', accmeasure='accabs', plotAccuracy = FALSE,meanOnly=TRUE, setPar = FALSE)
plotPES2(distributions=MS2$distributions3, dat=mileticvanmaanen2019exp2block2$dat, do.plot=FALSE, pp2=NULL, main1='Dataset 3', rtmeasure = 'rtabs', accmeasure='accabs', plotAccuracy = FALSE,meanOnly=TRUE, setPar = FALSE)

plotPES2(distributions=MS3$distributions4, dat=wagenmakers2008exp2$dat, pp2=NULL, main1='Dataset 4', shift=TRUE, rtmeasure = 'rtabs', accmeasure='accabs', plotAccuracy = FALSE,meanOnly=TRUE, setPar = FALSE)
plotPES2(distributions=MS2$distributions4, dat=wagenmakers2008exp2$dat, pp2=NULL, do.plot=FALSE, main1='Dataset 4', rtmeasure = 'rtabs', accmeasure='accabs', plotAccuracy = FALSE,meanOnly=TRUE, setPar = FALSE)
legend('topright', c('MS2-RDM', 'MS3-RDM'), lty=c(1,1), lwd=c(2,2), col=c(3,2), bty='n')
dev.off()
...
for(ftype in c('jpeg','pdf')) {
  fn = file.path(figures_dir, 'ER_MS3RDM_group_and_individual_dataset5.pdf')
