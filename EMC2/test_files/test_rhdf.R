...
file.remove('./pnassamples.h5')

## Test 1: Run each stage separately with stages longer than max_burn_iter and max_sample_iter
samplers <- run_samplers(samplers, stage='preburn', cores_per_chain = 5, iter=300, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5', particle_factor=20)
samplers <- loadH5(fn='pnassamples.h5')
samplers <- run_samplers(samplers, stage='burn', cores_per_chain = 1, iter=500, cores_for_chains = 1, verbose = T,
                       fileName='./pnassamples.h5', particle_factor=20)
samplers <- loadH5(fn='pnassamples.h5')
samplers <- run_samplers(samplers, stage='adapt', cores_per_chain = 1, iter=300, cores_for_chains = 1, verbose = T,
                         fileName='./pnassamples.h5', particle_factor=20)
samplers <- loadH5(fn='pnassamples.h5')
samplers <- shorten_chains_lowmem(samplers, verbose=TRUE)
samplers <- run_samplers(samplers, stage='sample', cores_per_chain = 5, iter=3000, cores_for_chains = 1, verbose = T,
                         fileName='./pnassamples.h5', particle_factor=20)
samplers <- loadH5(fn='pnassamples.h5')


## Test 2: Run run_emc with iter > 1e3
samplers <- make_samplers(dat2, design_B, type = "standard")
file.remove('./pnassamples.h5')
samplers <- run_emc(samplers, cores_per_chain = 7, iter=1200, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5', particle_factor=20)

## test 3: continue halfway through
#samplers <- make_samplers(dat2, design_B, type = "standard")
#file.remove('./pnassamples.h5')
samplers <- run_emc(samplers, stage = c('preburn', 'burn', 'adapt'), cores_per_chain = 7, iter=3000, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5', particle_factor=15)
samplers <- loadH5(fn='pnassamples.h5')
samplers <- shorten_chains_lowmem(samplers)
samplers <- run_emc(samplers, cores_per_chain = 1, iter=3000, cores_for_chains = 1, verbose = T,
                    fileName='./pnassamples.h5')



samplers <- run_emc(samplers, stage='preburn', preburn = 300, cores_per_chain = 5, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5')
samplers <- loadH5(fn='pnassamples.h5')

samplers <- run_emc(samplers, stage='burn', iter=700, cores_per_chain = 5, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5')
samplers <- loadH5(fn='pnassamples.h5')

samplers <- run_emc(samplers, stage='adapt', cores_per_chain = 5, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5')
samplers <- loadH5(fn='pnassamples.h5')

samplers <- run_emc(samplers, stage='sample', iter=7000, cores_per_chain = 5, cores_for_chains = 3, verbose = T,
                    fileName='./pnassamples.h5')
samplers <- loadH5(fn='pnassamples.h5')
