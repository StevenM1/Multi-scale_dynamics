  ...
  if(is.null(stage)){
    ...
    if (nstage["preburn"]==0) {
      ...
      if (nstage["sample"]>0){
        ...
        if(stage == 'preburn') {
          if(nstage['preburn'] >= preburn) stage <- 'burn'
        }
      }
    }
  }
...
run_samplers <- function(samplers, stage, iter = NULL, max_gd = NULL, mean_gd = NULL, min_es = 0, min_unique = 600,
  ...
  # set-up saving
  fileNameH5 <- NULL
  fileNameRData <- fileName
  if(!is.null(fileName)) {
    if(grepl("\\.h5$", fileName)) {
      fileNameH5 <- fileName
      fileNameRData <- gsub('\\.h5$', '.RData', fileName)
    }
  }

  attributes <- get_attributes(samplers)
  total_iters_stage <- chain_n_h5(fileName)[,stage][1]
  if(is.null(total_iters_stage)) total_iters_stage <- chain_n(samplers)[,stage][1]
  ...
  # unbind from all parent envs
  lapply(sys.frames(), function(x) rm(samplers, envir=x))
  samplers <- progress$samplers
  if(!is.null(fileNameH5)) {
    progress <- progress[names(progress)!='samplers']   # unbind so progress$samplers doesn't contain the *old* samplers once samplers has been updated
  }
  ...
  while(!progress$done){
    ...
    start.time <- Sys.time()
    samplers <- auto_mclapply(samplers,run_stages, stage = stage, iter= step_size,
    ...
    ## re-create reference to same object to reduce memory footprint
    for(i in 2:length(samplers)) samplers[[i]]$data <- samplers[[1]]$data

    if(!is.null(fileNameH5)) {
      ## append to H5 & reload here so we have *all* samples to do the progress checking with
      samplers <- append_and_reload(samplers, fileNameH5, fileNameRData, step_size)
      progress <- check_progress(samplers, stage, iter, max_gd, mean_gd, min_es, min_unique, max_trys, step_size, cores_per_chain,
                                 verbose, progress, total_iters_stage=total_iters_stage+step_size)
    } else {
      progress <- check_progress(samplers, stage, iter, max_gd, mean_gd, min_es, min_unique, max_trys, step_size, cores_per_chain,
                                 verbose, progress)
    }
    ...
    if(!is.null(fileNameH5)) {
      # already saved, shorten and continue
      samplers <- shorten_chains_lowmem(samplers)
      print(gc())  # print&see memory usage
      total_iters_stage <- chain_n_h5(fileNameH5)[,stage][1]
    } else if(!is.null(fileNameRData)) {
      ## Save RData
      fileNameRData <- fix_fileName(fileNameRData)
  ...
  #
  #     if(!is.null(fileNameH5)) {
  #       ## this is a hdf5 file, use new method
  #       ## check for existence
  ...
  #         if(nPost < step_size) {
  #           n_append <- step_size - (nPost-nPre)
  #         } else {
  #           n_append <- step_size
  #         }
  #         append_samplers_to_h5(samplers, select=n_append, fn=fileNameH5, last_select=TRUE)
  #
...
check_progress <- function (samplers, stage, iter, max_gd, mean_gd, min_es, min_unique,
                            max_trys, step_size, n_cores, verbose, progress = NULL, total_iters_stage=NULL)
{
  if(is.null(total_iters_stage)) total_iters_stage <- chain_n(samplers)[, stage][1]
  if (is.null(progress)) {
    iters_total <- 0
