  ...
  # if last_select applies scalar select from last iteration
{
  ...
  if(any(pmwg$nuisance)) {
    pmwg$sampler_nuis$samples$stage <- pmwg$samples$stage   # SM: overwrite these first
  ...
  stage <- pmwg$samples$stage[ok]   # for some reason, this is sometimes correctly and sometimes incorrectly filtered in filter_obj; so do it manually and add this workaround
  pmwg$samples <- base::rapply(pmwg$samples, f = function(x) filter_obj(x, filter_idx), how = "replace")
  pmwg$samples$stage <- stage
